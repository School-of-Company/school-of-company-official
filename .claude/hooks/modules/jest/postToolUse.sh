...
if [[ "$TOOL_NAME" == "Edit" ]] || [[ "$TOOL_NAME" == "Write" ]]; then
    ...
    case "$FILE_PATH" in
        *.js|*.ts|*.jsx|*.tsx)
            if ! compgen -G "$CWD/jest.config.*" > /dev/null 2>&1 && \
               ! compgen -G "jest.config.*" > /dev/null 2>&1; then
                if ! grep -q '"jest"' "$CWD/package.json" 2>/dev/null && \
                   ! grep -q '"jest"' "package.json" 2>/dev/null; then
                    exit 0
                fi
            fi
            JEST="$CWD/node_modules/.bin/jest"
            [[ -x "$JEST" ]] || JEST="node_modules/.bin/jest"
